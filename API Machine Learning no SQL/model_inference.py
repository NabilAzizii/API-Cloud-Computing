...
def inference(model_paths: list = None, data_points: list = None):
    """
    ...
    assert len(model_paths) == len(
        data_points
    ), "jumlah models dan data_poin tidak sesuai"
    predictions = []
...
if __name__ == "__main__":
    prediction, confidence = inference(
        ...
        [
            np.array(
                [[1, 100, 1.2, 1, 1]]
            ),  # kolom : [prestasi, nilai ujian, gaji ortu, punya kip SMA, status rumah]
            process_image("./rumah4.jpg"),
        ],
    )
